...
import json
import base64
from addresses import DC02_address, DC12_address
...
import csv
import sys
from addresses import DC02_address, DC12_address, Order_address, Order_address_source, PostCode_address, Output_DCx2_address, Output_DCx2_address_west
...
with open(DC02_address, newline='', encoding="shift_jis_2004") as f:
    r = csv.reader(f)
    data = [line for line in r]
with open(DC02_address,'w',newline='', encoding="shift_jis_2004") as f:
    w = csv.writer(f)
    w.writerow(header)
    w.writerows(data)  
...
#The following adds the header above to the first line of the file.
with open(DC12_address,newline='', encoding="shift_jis_2004") as f:
    r = csv.reader(f)
    data = [line for line in r]
with open(DC12_address,'w',newline='', encoding="shift_jis_2004") as f:
    w = csv.writer(f)
    w.writerow(header)
    w.writerows(data)  

df_inv_east = pd.read_csv(DC02_address, usecols=["DC_Name","stock_no","available_qty"])
...
df_inv_west = pd.read_csv(DC12_address, usecols=["DC_Name","stock_no","available_qty"])
...
name_east_file = Output_DCx2_address + 'East_orders' + timestampStr +'.csv'
name_west_file = Output_DCx2_address_west + 'West_orders' + timestampStr +'.csv'
name_incom_file = Output_DCx2_address + 'Incomplete_orders' + timestampStr +'.csv' 
