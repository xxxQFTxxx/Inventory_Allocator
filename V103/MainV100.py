...
with open("./input/Inventory_East.csv",newline='', encoding="shift_jis_2004") as f:
    r = csv.reader(f)
    data = [line for line in r]
with open('./input/Inventory_EastWithHeader.csv','w',newline='',encoding="shift_jis_2004") as f:
    w = csv.writer(f)
    w.writerow(header)
    w.writerows(data)  

#The following adds the header above to the first line of the file.
with open("./input/Inventory_West.csv",newline='', encoding="shift_jis_2004") as f:
    r = csv.reader(f)
    data = [line for line in r]
with open('./input/Inventory_WestWithHeader.csv','w',newline='',encoding="shift_jis_2004") as f:
    w = csv.writer(f)
    w.writerow(header)
    w.writerows(data)  

df_inv_east = pd.read_csv("./input/Inventory_EastWithHeader.csv", usecols=["DC_Name","stock_no","available_qty"])
df_inv_east.columns=["InverntoryName",  "Item code", "NumItemsAvailable"] #renames the columns

df_inv_west = pd.read_csv("./input/Inventory_WestWithHeader.csv", usecols=["DC_Name","stock_no","available_qty"])
...
#The following adds the header above to the first line of the file.
with open("./input/Order.csv",newline='', encoding="shift_jis_2004") as f:
    r = csv.reader(f)
    data = [line for line in r]
with open('./input/OrderWithHeader.csv','w',newline='',encoding="shift_jis_2004") as f:
    w = csv.writer(f)
    w.writerow(header_order)
    w.writerows(data)

df_order = pd.read_csv("./input/OrderWithHeader.csv", encoding="shift_jis_2004",converters={'Receiver_Zip_Code': lambda x: x.strip('”'),'Shipping_Order_No': lambda x: x.strip('”'),'Stock_No': lambda x: x.strip('”'),'Quantity': lambda x: int(x.strip('”')) })
...
df_postal = pd.read_csv("./input/Zip_Code East West List.csv", converters={'Zip_Code 3degit': lambda x: str(x)}, usecols= ["Zip_Code 3degit", "East/West"]) #Reading the Post code file
df_postal.columns=["PostCode", "East_West"] #renames the columns
df_postal = df_postal.drop_duplicates(keep='first',ignore_index = True)
...
name_east_file = './output/East_orders' + timestampStr +'.csv'
name_west_file = './output/West_orders' + timestampStr +'.csv'
name_incom_file = './output/Incomplete_orders' + timestampStr +'.csv'
